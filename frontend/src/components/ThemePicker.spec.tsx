...
import * as themeConfigModule from "../config/theme.config";
...
describe("ThemePicker component", () => {
  ...
  it("does not render when showThemePicker is false", () => {
    render(
      <ThemeProvider>
        <ThemePicker />
      </ThemeProvider>
    ...
    expect(screen.queryByTestId("theme-picker-btn")).not.toBeInTheDocument();
  });

  describe("when showThemePicker is enabled", () => {
    beforeEach(() => {
      vi.spyOn(themeConfigModule, "themeConfig", "get").mockReturnValue({
        defaultPalette: "electric-cyan",
    ...
    it("renders picker trigger button with current palette name", () => {
      ...
      expect(btn).toHaveTextContent("Electric Cyan");
    });
